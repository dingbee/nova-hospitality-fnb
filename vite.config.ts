...
export default defineConfig({
  test: {
    // e2e/ and e2e-auth/ hold Playwright specs (run via `npx playwright
    // test`), not vitest tests — without this, vitest's default *.spec.ts
    // glob picks them up too and fails, since they use @playwright/test's
    // `test`, not vitest's.
    exclude: ["**/node_modules/**", "e2e/**", "e2e-auth/**"],
  },
  plugins: [
    VitePWA({
      manifest: false,
