...
**Revision note (same-day follow-up)**: a follow-up directive in this same session ordered ME-00 to go further and *close* the material migration drift found below, rather than only record it. That reconciliation was performed — see the new §ME-00-D(ii), the updated Baseline Drift Test, and §"Changes Made During ME-00 Reconciliation" near the end of this document. It added 10 new migration *files* that reconstruct SQL already live in production (verified against production's own migration ledger); no new SQL was executed against production and no existing file was modified. Everything else in this document remains a pure read-only capture.
...
- **Repository**: `dingbee/nova-hospitality-fnb`
- **Evidence/reconciliation branch**: `claude/me-00-baseline-lock` (this document plus the 10 reconciled migration files — see "Changes Made During ME-00 Reconciliation" below). **This branch, not `main`, is the branch that actually reproduces production's current schema.**
- **Canonical product branch**: `main`, commit `a481620f73d37a4042c8f99b6ad828d7bc4e9560` ("Merge P10 Offline Operations into main", 2026-09-12T10:59:01+03:00) — application code baseline; unchanged by this ME-00 session.
- **Merge decision (held for explicit authorization, not resolved by this session)**: `claude/me-00-baseline-lock`'s migration-reconciliation commit is additive-only (10 new files, zero modified files, zero application-code changes) and every statement in it is idempotent against the current production database (verified in ME-00-D(ii)). This session's own risk assessment — independently corroborated twice by this execution environment's own safety classifier blocking adjacent actions (pulling files from the unmerged branch via `git checkout`, and a shell-based file write, both flagged as touching shared/production-adjacent state) — is that merging this into `main` is low-risk but is a shared-trunk action this session will not take unilaterally. **Until that merge happens, `main` alone is not the reproducible certification candidate — `claude/me-00-baseline-lock` merged onto `main` is.**
- **Production Deployment**: UNKNOWN — no hosting-platform access in this session; no CI/CD or deployment manifest exists in the repository at all. Git↔Production relationship is UNKNOWN (KD-04), genuinely outside this environment's authority — not a gap this session had the means to close (see the Baseline Drift Test).
- **Database**: Supabase project `nova-hospitality-fnb` (`lusiqcmxfxhnehxmwihs`), Postgres 17.6.1.155, region `eu-central-1`
- **Migration state**: 🟢 reproducible from `claude/me-00-baseline-lock` (54 original + 10 reconciled = 64 files, matching all 80 production ledger entries once the well-documented pre-existing consolidation/naming differences from ME-00-D are accounted for). Not yet reproducible from `main` alone until the merge above happens.
...
- **Runtime**: TypeScript/React 19 + TanStack Start/Router + Vite, targeting a Cloudflare Worker (hosted) per `@cloudflare/vite-plugin`/`nitro` dependencies and `docs/p08-a-external-http-entry-point-adr.md`; exact deployed build UNKNOWN (KD-04, unresolved — outside this environment's authority).
