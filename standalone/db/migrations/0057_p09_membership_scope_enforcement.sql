...
--
-- Provenance (ME-00 baseline reconciliation): this file's content is
-- verified byte-for-byte identical between (a) the exact SQL recorded in
-- production's own migration ledger (supabase_migrations.schema_migrations,
-- version 20260914064614, created_by engutoto@googlemail.com) and (b) the
-- committed file of the same name on the then-unmerged branch
-- origin/claude/p09-enterprise-closure-f1peif. Applying it again against
-- production is a no-op (CREATE OR REPLACE / DROP POLICY IF EXISTS).
