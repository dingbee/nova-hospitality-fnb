-- LexiBite POS Staff PIN Sessions
-- Native operational credential layered on the existing restaurant_members/RBAC identity.
...
begin
  ...
  if not exists (
    ...
    raise exception 'Not authorized to manage POS PINs';
  end if;

  update public.restaurant_members
     set pos_pin_hash = crypt(p_pin, gen_salt('bf', 10)),
...
declare
  target public.restaurant_members%rowtype;
begin
  select * into target from public.restaurant_members
   where id = p_member_id and tenant_id = p_tenant_id;
  if not found then raise exception 'Staff member not found'; end if;
...
create or replace function public.restaurant_start_pos_session(
  p_tenant_id uuid,
  p_property_id uuid,
...
begin
  ...
  where tenant_id = p_tenant_id
    ...
    and pos_pin_enabled = true
  order by (property_id is null) asc
  limit 1;

  if not found or member.pos_pin_hash is null or crypt(p_pin, member.pos_pin_hash) <> member.pos_pin_hash then
  ...
  update public.restaurant_pos_sessions
   ...
   where tenant_id = p_tenant_id
     ...
     and terminal_id = coalesce(nullif(p_terminal_id,''),'pos-web')
     and active = true;
...
revoke all on function public.restaurant_start_pos_session(uuid,uuid,uuid,text,text) from public;
...
grant execute on function public.restaurant_start_pos_session(uuid,uuid,uuid,text,text) to authenticated;
