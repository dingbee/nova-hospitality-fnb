-- Fix pgcrypto resolution for Supabase, where pgcrypto functions live in extensions.
-- POS PIN functions must explicitly use extensions.crypt/gen_salt.
...
set search_path = public, extensions, pg_temp
as $$
declare member public.restaurant_members%rowtype; match_count integer; session_id uuid;
begin
  if auth.uid() is null then raise exception 'Authentication required'; end if;
