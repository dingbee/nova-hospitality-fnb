...
const USER_TENANT_WIDE = "user-tenant-wide"; // owner, tenant-wide
const USER_NONE = "user-none"; // no membership at all

function makeFakeSupabase() {
  const members = [
    { tenant_id: TENANT_A, user_id: USER_SCOPED_A1, role: "restaurant_manager", property_id: PROPERTY_A1 },
    ...
    from(table: string) {
      ...
      const api: any = {
        ...
        async then(resolve: any) {
          const rows = table === "restaurant_members" ? members : table === "restaurant_locations" ? locations : [];
...
describe("P0 property isolation — assertCapability with scope", () => {
  ...
  it("omitting scope entirely preserves the original tenant-only behaviour — a scoped member's role still passes with no property check", async () => {
    const sb = makeFakeSupabase();
    await expect(assertCapability(sb, USER_SCOPED_A1, TENANT_A, "sales.manage")).resolves.toBeUndefined();
  });
});
