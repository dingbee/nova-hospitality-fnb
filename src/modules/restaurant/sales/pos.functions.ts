...
export const posBoardFn = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data, context }) => {
    ...
    const actorId = await (await import("./pos-session.server")).resolvePosActor(context.supabase, context.userId, data.posSessionId, data.tenantId, (data as { propertyId?: string }).propertyId);
