...
import { unitScale } from "@/modules/restaurant/catalog/parse";
...
export function UnitsPanel({ tenantId, data }: { tenantId: string; data: MasterData }) {
  ...
  function openEdit(id: string) {
    ...
    setForm({
      ...
      baseUnitId: u.base_unit_id ?? null,
      factor: u.factor ?? 1,
    });
    setOpen(true);
  }
  ...
  return (
    <SectionCard
      title="Units"
      description="Units of measure with base-unit conversion factors for stock and recipes."
    >
      ...
      >
        <FieldRow>
          <Field
            ...
            hint={
              !editing
                ? "kg, g, l, ml and pc auto-fill the correct dimension and factor below."
                : undefined
            }
          >
            <Input
              ...
              onChange={(e) => {
                ...
                setForm((f) => {
                  // Only autofill while creating a brand-new unit — never
                  // overwrite an existing unit's already-configured values.
                  if (editing) return { ...f, code };
                  const scale = unitScale(code);
                  return scale
                    ? { ...f, code, dimension: scale.dimension, factor: scale.factor }
                    : { ...f, code };
                });
              }}
              required
        ...
        <Field label="Base unit" hint="Leave empty if this is itself a base unit.">
          <SearchSelect
            ...
            onChange={(v) => setForm((f) => ({ ...f, baseUnitId: v }))}
            placeholder="None"
          />
        </Field>
