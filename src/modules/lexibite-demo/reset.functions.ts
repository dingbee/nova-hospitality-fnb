...
const revokeDemoSessionSchema = z.object({ sessionId: z.string().uuid() });
...
/** Commercial-admin only (checked inside revokeDemoSession, and again at the SQL level). Never public. */
export const revokeDemoSessionFn = createServerFn({ method: "POST" })
  .middleware([requireSupabaseAuth])
  .inputValidator((d: unknown) => revokeDemoSessionSchema.parse(d))
  .handler(async ({ data, context }) => {
    const mod = await import("./reset.server");
    return mod.revokeDemoSession(context.supabase, context.userId, data.sessionId);
  });
