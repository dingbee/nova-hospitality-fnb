...
export const AI_GATEWAY_URL = process.env["NOVA_AI_GATEWAY_URL"] ?? "https://api.openai.com/v1/chat/completions";
...
export interface AiGatewayResult { content: string; latencyMs: number; model: string; inputTokens?: number; outputTokens?: number; }
...
export async function callAiGateway(opts: AiGatewayCallOptions): Promise<AiGatewayResult> {
  ...
  let res: Response;
  try {
    const systemForResponses = opts.jsonMode
      ? `${opts.system}\n\nOutput requirement: respond with valid JSON only. The response must be JSON.`
      : opts.system;
    const body = protocol === "responses"
      ? { model, instructions: systemForResponses, input: opts.history?.length ? [...opts.history.map((message) => ({ role: message.role, content: message.content })), { role: "user", content: opts.user }] : opts.user, ...(opts.jsonMode ? { text: { format: { type: "json_object" } } } : {}) }
      : { model, messages: [{ role: "system", content: opts.system }, ...(opts.history ?? []), { role: "user", content: opts.user }], ...(opts.jsonMode ? { response_format: { type: "json_object" } } : {}) };
    res = await fetch(url, { method: "POST", headers: { "Content-Type": "application/json", Authorization: `Bearer ${key}` }, body: JSON.stringify(body), signal: controller.signal });
  } catch (err) {
  ...
  return { content: protocol === "responses" ? extractResponsesText(json) : (json.choices?.[0]?.message?.content ?? ""), latencyMs, model, inputTokens: json.usage?.input_tokens ?? json.usage?.prompt_tokens, outputTokens: json.usage?.output_tokens ?? json.usage?.completion_tokens };
}
