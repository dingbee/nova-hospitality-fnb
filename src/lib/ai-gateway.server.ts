...
export async function callAiGateway(opts: AiGatewayCallOptions): Promise<AiGatewayResult> {
  ...
  try {
    const systemForResponses = opts.jsonMode
      ...
      : opts.system;
    // The Responses API validates the literal word "json" against the
    // `input` field itself when text.format.type is "json_object" — having
    // it only in `instructions` (as above) does not satisfy that check and
    // fails with a 400 ("Response input messages must contain the word
    // 'json' in some form..."). The reminder must be appended to the actual
    // user-turn content that goes into `input`.
    const userForResponses = opts.jsonMode ? `${opts.user}\n\n(Respond with JSON.)` : opts.user;
    const body =
      protocol === "responses"
        ? {
            model,
            instructions: systemForResponses,
            input: opts.history?.length
              ? [
                  ...
                  { role: "user", content: userForResponses },
                ]
              : userForResponses,
